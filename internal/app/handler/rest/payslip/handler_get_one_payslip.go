...
func (h *handler) GetOnePayslip(c echo.Context) (err error) {
	userUUID := c.Get("user_uuid").(string)

	var req payslip.GetOnePayslipRequest
	...
	resp, err := h.payslipUsecase.GetOnePayslip(c.Request().Context(), req)
	if err != nil {
		return
	}
